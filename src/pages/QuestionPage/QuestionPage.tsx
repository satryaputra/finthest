...
export default function QuestionPage() {
  ...
  const { mutate } = useMutation({
    mutationFn: async () => {
      const { salary, loan, month, savingMoney, wish, price } =
        metodth.getValues();
      const loginResponse = await axios.post(
        `http://localhost:8000/api/calculation${id}`,
        {
          salary,
          loan,
          month,
    ...
    <div>
      ...
      <div className="px-6 py-4 overflow-y-hidden">
        <h1 className="text-lg font-normal mb-5">
          Silahkan isi beberapa pertanyaan berikut ini
        </h1>
        <div className="w-full h-[24rem] overflow-y-auto border-t-2">
          <FormProvider {...metodth}>
            ...
            >
              <div>
                <Input name="salary" placeholder="Gaji per bulan" type="text" />
              </div>
              <div>
                ...
                {loanValue === "true" && (
                  <div className="">
                    <Input
                      name="month"
                      placeholder="Eetimasi Tanggal Lunas"
                      type="text"
                    />
                  </div>
                )}
                ...
                <div className="flex items-center gap-4">
                  ...
                  {wishValue === "true" && (
                    <div className="mb-8 w-full">
                      <Input
                        name="wish"
                        placeholder="Tuliskan wishlist anda"
                        type="text"
                      />
                    </div>
                  )}
                    ...
                    <Popover open={open} onOpenChange={setOpen}>
                      <PopoverTrigger asChild>
                        ...
                        >
                          {year
                            ? yearsPlan.find(
                                (yearsPlan) => yearsPlan.value === year
                              )?.label
                            : "Tahun"}
              ...
              {wishValue === "true" && (
                <div className="mb-8">
                  <Input
                    name="price"
                    placeholder="Tuliskan harga dari wishlist anda"
                    type="text"
                  />
                </div>
              )}
              <div className="absolute bottom-0 w-[93%] pr-6 ">
                <Button
                  type="submit"
                  className="bg-primary py-2 rounded w-full text-white text-lg"
                >
                  Kalkulasi
                </Button>
              </div>
